...
function DrawingApp() {
      ...
      .forEach((layer) => {
        if (!layer.hidden && layer.elements) {
          layer.elements
            .slice()
            .reverse()
            .forEach((element) => drawElement(roughCanvas, context, element));
        }
      });
    ...
    layers.map((layer) => {
      if (layer.hidden)
        document
          ...
          .classList.add("hiddenLayer");
      else
        document
          .getElementById(`layer-${layer.id}`)
          .classList.remove("hiddenLayer");
    });
  }, [layers]);
  ...
  const getData = async () => {
    ...
    try {
      const data = await stocotoonAPI.get(`/script/${PageId}`, {
        headers: {
          Authorization: `Bearer ${session.UserToken}`,
        },
      });
      // console.log(JSON.parse(data.data.content).script);
      setScript(data.data.content.script);
      // setScript(JSON.parse(data.data.content).script);
    } catch (error) {
      console.log(error);
    }
  };
  ...
  const handleMouseDown = (event) => {
    ...
    if (tool === "selection" || tool === "scissors") {
      ...
      if (element) {
        const index = selectedLayer.elements.findIndex(
          ({ id }) => id === element.id
        );

        const elementsCopy = selectedLayer.elements.filter(
          (_, i) => i !== index
        );
      ...
      ["pencil", "rectangle", "ellipse", "line", "text"].includes(tool)
    ) {
  ...
  const handleMouseMove = (event) => {
    if (selectedLayer.hidden) {
      ...
      return;
    } else document.getElementById("canvas").style.cursor = "crosshair";
    ...
    if (tool === "selection") {
      ...
      if (element)
        document.getElementById("canvas").style.cursor = cursorForPosition(
          element.position
        );
    }
    if (tool === "select" && selectedArea.x1 && selectedElements.length > 0) {
      const element = getElementAtPosition(mouseX, mouseY, [selectedArea]);
      if (element) document.getElementById("canvas").style.cursor = "move";
    }
    if (tool === "scissors") {
      const element = getElementAtPosition(
        mouseX,
      ...
      if (element) document.getElementById("canvas").style.cursor = "pointer";
      return;
    }
    ...
    if (action === "drawing") {
      ...
      selectedElements.map((selected) => {
        if (selected.type === "pencil") {
          ...
          const { id, x1, y1, x2, y2, type, offsetX, offsetY, configs } =
            selected;
          const width = x2 - x1;
          const height = y2 - y1;
      ...
      const { id, type, configs, position, ...coordinates } =
        selectedElements[0];
  ...
  const handleMouseUp = (event) => {
    ...
    if (
      ...
      console.log(-1);
      if (
        (selectedElements[0].offsetX !== undefined &&
          selectedElements[0].offsetY !== undefined &&
          mouseX - selectedElements[0].offsetX === selectedElements[0].x1 &&
          mouseY - selectedElements[0].offsetY === selectedElements[0].y1) ||
        (selectedElements[0].xOffsets &&
          selectedElements[0].yOffsets &&
          selectedElements[0].points &&
          mouseX - selectedElements[0].xOffsets[0] ===
            selectedElements[0].points[0].x &&
          mouseY - selectedElements[0].yOffsets[0] ===
            selectedElements[0].points[0].y)
      ) {
        console.log(1);
        if (tool === "selection" && selectedElements[0].type === "text") {
          ...
          const elementsCopy = [...selectedLayer.elements];
          const newElements = elementsCopy.filter(
            (element) => element.id !== selectedElements[0].id
          );
          setSelectedLayer((selectedLayer) => ({
            ...selectedLayer,
            elements: newElements,
      ...
      setSelectedArea(newSelection);
      const newSelectedElements = selectElements(
        x1,
        y1,
        x2,
        y2,
    ...
    <div>
      <div className="drawing-app">
        ...
        <div className="tools-container">
          <button className="draw-button tool" onClick={() => navigate(-1)}>
            <img src={icons.ComeBack}></img>
          </button>
          <button className="draw-button tool" onClick={saveProject}>
          ...
          <input
            ...
            id="stroke"
            onChange={(e) =>
              setToolConfigs({ ...toolConfigs, stroke: e.target.value })
            }
          />
          ...
          <input
            ...
            onChange={(e) =>
              setToolConfigs({ ...toolConfigs, strokeWidth: e.target.value })
            }
          />
          <br />
          ...
          {isFilled && (
            <input
              ...
              id="fill"
              onChange={(e) => {
                setToolConfigs({ ...toolConfigs, fill: e.target.value });
              }}
            />
          )}
          <button
            className={
              historyIndex === 0 ? "draw-button unable" : "draw-button"
            }
            onClick={undo}
            style={{ scale: "0.8" }}
          >
          ...
          <div className="layers-container">
            <div className="layers-box">
                ...
                {layers.map((layer, index) => (
                  ...
                  >
                    ...
                    >
                      <img
                        className={
                          layer.hidden ? "viewIcon hidden" : "viewIcon"
                        }
                        src={icons.View}
                      />
                    </button>
              ...
              <div className="layer-buttons" key={selectedLayer.id}>
                ...
                <button
                  onClick={() => upLayer(selectedLayer.id)}
                  className="draw-button"
                >
                  <img src={icons.Arrow} alt="DeleteLayer" />
                </button>
                <button
                  onClick={() => downLayer(selectedLayer.id)}
                  className="draw-button"
                >
                  <img
                    src={icons.Arrow}
